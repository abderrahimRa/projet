...
export class SecondaryComponent implements AfterViewInit {
  ...
  private initStateChart() {
    ...
    this.stateChart = new Chart(ctx, {
      type: 'bar',
      data: {
        labels: ['High Schools', 'Classes', 'Students Enrolled', 'Half-board Programs'],
        datasets: [
          {
            label: 'Public',
            data: [
              this.tableData.reduce((sum, row) => sum + row.pubNumHighschools, 0),
          ...
          },
          {
            label: 'Private',
            data: [
              this.tableData.reduce((sum, row) => sum + row.priNumHighschools, 0),
  ...
  private initDairaComparisonChart() {
    ...
    this.dairaComparisonChart = new Chart(ctx, {
      type: 'bar',
      data: {
        labels: ['High Schools', 'Classes', 'Students', 'Half-board'],
        datasets: [
          {
            label: 'Bouira - Public',
            data: [
              this.FirstpubNumHighschools,
  ...
  calculateFirstTotals() {
    this.FirstpubNumHighschools = this.FirstData.reduce((sum, row) => sum + +row.pubNumHighschools, 0);
    ...
    this.FirstPriNumHighschools = this.FirstData.reduce((sum, row) => sum + +row.priNumHighschools, 0);
  ...
  calculateSecondTotals() {
    this.SecondpubNumHighschools = this.SecondData.reduce((sum, row) => sum + +row.pubNumHighschools, 0);
    ...
    this.SecondPriNumHighschools = this.SecondData.reduce((sum, row) => sum + +row.priNumHighschools, 0);
  ...
  exportTableToExcel1() {
    ...
    if (table) {
      ...
      XLSX.writeFile(wb, 'Secondary.xlsx');
    } else {
