...
@Component({
  ...
  templateUrl: './cfpa.component.html',
  styleUrls: ['./cfpa.component.css'],
  standalone: false,
})

export class CFPAComponent implements AfterViewInit {
  isTableHidden = false;
  ...
  constructor(private http: HttpClient, private router: Router) {}
         ...
         calculateFirstTotals() {
           this.FirstnumCFPAs = this.FirstData.reduce((sum, row) => sum + +row.numCFPAs, 0);
         ...
         calculateSecondTotals() {
           this.SecondnumCFPAs = this.SecondData.reduce((sum, row) => sum + +row.numCFPAs, 0);
  ...
  private initStateChart() {
    ...
    this.stateChart = new Chart(ctx, {
      type: 'bar',
      data: {
        labels: ['CFPAs', 'Teaching Places', 'Trainees'],
        datasets: [{
          label: 'State Total',
      ...
      options: this.getChartOptions('CFPA Facilities Overview')
    });
  }
  ...
  private initDairaComparisonChart() {
    ...
    this.dairaComparisonChart = new Chart(ctx, {
      ...
      options: this.getChartOptions('Daira Comparison: CFPA Facilities')
    });
  }
  ...
  private initUtilizationChart() {
    ...
    this.utilizationChart = new Chart(ctx, {
      ...
      options: this.getChartOptions('CFPA Capacity Utilization')
    });
  }

  private getStateDataTotals(): number[] {
    return [
      this.tableData.reduce((sum, row) => sum + row.numCFPAs, 0),
