...
@Component({
  ...
  templateUrl: './insfp.component.html',
  styleUrls: ['./insfp.component.css'],
  standalone: false,
})

export class INSFPComponent implements AfterViewInit {
  isTableHidden = false;
  ...
  constructor(private http: HttpClient, private router: Router) {}
      ...
      calculateFirstTotals() {
        this.FirstnumINSFPs = this.FirstData.reduce((sum, row) => sum + +row.numINSFPs, 0);
      ...
      calculateSecondTotals() {
        this.SecondnumINSFPs = this.SecondData.reduce((sum, row) => sum + +row.numINSFPs, 0);
  ...
  private initStateChart() {
    ...
    this.stateChart = new Chart(ctx, {
      type: 'bar',
      data: {
        labels: ['INSFPs', 'Teaching Places', 'Trainees'],
        datasets: [{
          label: 'State Total',
      ...
      options: this.getChartOptions('INSFP Facilities Overview')
    });
  }
  ...
  private initDairaComparisonChart() {
    ...
    this.dairaComparisonChart = new Chart(ctx, {
      ...
      options: this.getChartOptions('Daira Comparison: INSPF Facilities')
    });
  }
  ...
  private initUtilizationChart() {
    ...
    this.utilizationChart = new Chart(ctx, {
      ...
      options: this.getChartOptions('INSPF Capacity Utilization')
    });
  }

  private getStateDataTotals(): number[] {
    return [
      this.tableData.reduce((sum, row) => sum + row.numINSFPs, 0),
