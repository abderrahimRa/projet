...
export class UniversitiesComponent implements AfterViewInit {
  ...
  private initStateChart() {
    ...
    this.stateChart = new Chart(ctx, {
      type: 'bar',
      data: {
        labels: ['Universities', 'Teaching Places', 'Enrolled Students', 'Professors'],
        datasets: [{
          label: 'State Total',
      ...
      options: this.getChartOptions('Universities Overview')
    });
  }
  ...
  private getStateDataTotals(): number[] {
    return [
      this.tableData.reduce((sum, row) => sum + row.numUniversities, 0),
