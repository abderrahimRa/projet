...
@Component({
  ...
  styleUrls: ['./activity-zone.component.css'],
  standalone: false,
})
   ...
   constructor(private http: HttpClient, private router: Router) {}
    ...
    let newValue = inputElement.innerText;
       ...
       calculateFirstTotals() {
         this.FirstNumActZone = this.FirstData.reduce((sum, row) => sum + +row.numActZone, 0);
       ...
       calculateSecondTotals() {
         this.SecondNumActZone = this.SecondData.reduce((sum, row) => sum + +row.numActZone, 0);
